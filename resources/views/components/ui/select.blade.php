...
<el-select id="{{ $id }}" name="{{ $name }}" value="{{ $value }}" class="mt-2 block"
    ...
    <button type="button"
        ...
        <el-selectedcontent class="col-start-1 row-start-1 flex items-center gap-3 pr-6">
            <span class="block truncate">{{ ucfirst($value) }}</span>
        </el-selectedcontent>
    ...
    <el-options anchor="bottom start" popover
        ...
        @foreach ($options as $v => $display)
            <el-option value="{{ $v }}"
                class="group/option relative block cursor-default py-2 pr-9 pl-3 text-gray-900 select-none focus:bg-indigo-600 focus:text-white focus:outline-hidden">
                <div class="flex items-center">
                    <span
                        class="block truncate font-normal group-aria-selected/option:font-semibold">{{ ucfirst($display) }}</span>
                </div>
                <span
